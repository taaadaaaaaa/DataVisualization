...
function waterfallChart(data)
{
    ...
    const y = d3.scaleLinear()
        .domain([210000, d3.max(data, (d) => +d.end) *1.02 ])
        .range([height, 0]);
...
function pieChart(data, year)
{
    ...
    pieSvg.append("text")
        ...
        .attr("text-anchor", "middle")
        .text(`Immigration Reasons to Canada in ${year}`)
        .style("font-weight", 700);
};
...
function pieChartData(year)
{
    ...
    return d3.csv("./csv/CanadaData1.csv").then(function (data)
    {
        ...
        const extractedData =
        {
            Economic: parseFloat(filteredData.economic),
            Family: parseFloat(filteredData.family),
            Refugees: parseFloat(filteredData.refugees),
            Other: parseFloat(filteredData.other),
        };
